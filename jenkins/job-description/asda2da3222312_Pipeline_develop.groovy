...
def job_config = [
        job: [
                name: "asda2da3222312_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'asda2da3222312_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asda2da3222312_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asda2da3222312.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asda2da3222312", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asda2da3222312_Pipeline_develop");

job.with {

    authenticationToken('6J2psGTYNMAKB4MP2f9uQITctz0b2g7S')
